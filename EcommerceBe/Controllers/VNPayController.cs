    ...
    public class PaymentController : ControllerBase
    {
        ...
        public async Task<IActionResult> PaymentCallback()
        {
            ...
            catch (Exception ex)
            {
                ...
                return Redirect($"{_configuration["Frontend:BaseUrl"] ?? "http://localhost:3000"}/payment-callback?status=error&error=system_error");
            }
        }
        ...
        public async Task<IActionResult> PaymentIPN()
        {
            try
            {
                // ✅ FIX: IPN thường gửi dữ liệu qua POST form, nhưng VNPay có thể gửi qua query string
